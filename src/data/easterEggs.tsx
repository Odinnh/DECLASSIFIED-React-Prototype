...
export const EasterEggStore: IMisc = {
	[MapIds.dieMaschine]: [
		...
		new MiscMarker(``, EETypes.interactable, [174.74870507815817, 257.415635207116], `Crack in the wall where you can remotely activate the D.I.E Wonder Weapon after acquiring the D.I.E remote.`),
		new MiscMarker(``, EETypes.questCollectible, [196.29722988085663, 307.495182630813], `1 of 3 Aetherscope Parts, one will spawn randomly when entering the Dark Aether, dogs spawn after picking it up.`),
		new MiscMarker(``, EETypes.questCollectible, [356.6938648779643, 343.9768501463755], `1 of 3 Aetherscope Parts, one will spawn randomly when entering the Dark Aether, dogs spawn after picking it up.`),
		...
		new MiscMarker(``, EETypes.questCollectible, [314.69833497058926, 344.089550203993], `Box containing a fuse used in the Thermophasic upgrade steps, hidden under the plane nose, inaccessible unless in the Dark Aether.`),
		new MiscMarker(``, EETypes.questCollectible, [242.97643511037063, 255.9881891661138], `Crate containing the vial used in the Cryo upgrade steps. Shoot the crate with the D.I.E to knock it down.`),
		new MiscMarker(``, EETypes.questCollectible, [256.63368476295705, 206.02147315077295], `Tree fungus used in the Cryo upgrade steps.\nHave a Megaton shoot the fungus to activate it.\nInteract whilst holding the vial to place it underneath the fungus and fill the vial.`),
		new MiscMarker(``, EETypes.questCollectible, [258.3778460127693, 335.23982709555946], `1 of 3 Crystals used in the Electrobolt upgrade steps, use the alt fire of the D.I.E to collect the energy, then run down to the upgrade box and shoot it to progress.`),
		new MiscMarker(``, EETypes.questCollectible, [377.14480548250435, 224.28934027831147], `1 of 3 Crystals used in the Electrobolt upgrade steps, use the alt fire of the D.I.E to collect the energy, then run down to the upgrade box and shoot it to progress.`),
		new MiscMarker(``, EETypes.questCollectible, [258.5949702531161, 173.9165165178388], `1 of 3 Crystals used in the Electrobolt upgrade steps, use the alt fire of the D.I.E to collect the energy, then run down to the upgrade box and shoot it to progress.`),
	...
	[MapIds.dieMaschineUnderground]: [
		...
		new MiscMarker(`1m5tF`, EETypes.interactable, [147.39538299834174, 388.52809723485626], `Downstairs, hidden behind the top left of the zombie spawn window.\n1 of 5 blue orbs that you can shoot for a bonus chest.`),
		new MiscMarker(`PxlsM`, EETypes.interactable, [76.10682851697437, 358.63289696847636], `1 of 5 blue orbs that you can shoot for a bonus chest.`),
		new MiscMarker(`ONsCv`, EETypes.interactable, [201.05343475851072, 301.68965836584806], `High up on the wall.\n1 of 5 blue orbs that you can shoot for a bonus chest.`),
		new MiscMarker(`RFgwR`, EETypes.interactable, [221.09307449751262, 347.24424924795073], `1 of 5 blue orbs that you can shoot for a bonus chest.`),
		new MiscMarker(`7TACp`, EETypes.interactable, [134.79653471600014, 401.6474904867542], `Hidden behind the grating.\n1 of 5 blue orbs that you can shoot for a bonus chest.`),
		new MiscMarker(``, EETypes.interactable, [273.33595868320293, 353.86231995501794], `A portal that appears after shooting all 4 canisters of the containment device with the corresponding D.I.E upgrade.`),
		new MiscMarker(`5M9A0`, EETypes.questCollectible, [211.50810943751497, 233.52599646325353], `D.I.E upgrade chest, requires the fuse and can only be interacted with when in the Dark Aether. Re-appears as open, above ground, on the back of the truck.`),
		new MiscMarker(``, EETypes.questCollectible, [198.7906869627428, 178.82125886330365], `Plaguehound Gas condenser.\nInteract whilst holding the canister to place it, then kill a Plaguehound near it to fill it with gas.`),
		new MiscMarker(``, EETypes.questCollectible, [324.47444670325746, 318.8136922248336], `1 of 3 Orbs that spawn in the Dark Aether after acquiring the Aetherscope. Interact with it to trigger the ghostly figure dialogue.`),
		new MiscMarker(``, EETypes.questCollectible, [235.5479859749598, 273.1385305408606], `1 of 3 Orbs that spawn in the Dark Aether after acquiring the Aetherscope. Interact with it to trigger the ghostly figure dialogue.`),
		new MiscMarker(``, EETypes.questCollectible, [181.93046504375246, 348.6110192429405], `1 of 3 Orbs that spawn in the Dark Aether after acquiring the Aetherscope. Interact with it to trigger the ghostly figure dialogue.`),
		new MiscMarker(``, EETypes.questCollectible, [215.23791437285269, 305.50159134986285], `1 of 3 Aetherscope Parts, one will spawn randomly when entering the Dark Aether, dogs spawn after picking it up.`),
		new MiscMarker(``, EETypes.questCollectible, [339.1999615318702, 326.4211594207544], `Vogel's Computer.\n After activating all 3 ghostly dialogues, interact with it 3 times to insert the password.`),
		new MiscMarker(``, EETypes.questCollectible, [329.39656431710233, 325.66999226629395], `Ghostly dialogue that appears after entering the portal that appears after shooting after shooting all 4 canisters of the containment device.`),
		...
		new MiscMarker(``, EETypes.clue, [310.66531882982684, 339.46395788705297], `Containment device on the roof with four canisters pointing down. Shoot each one with the corresponding elemental version of the D.I.E.`),
	],
	...
	[MapIds.mauerDerTotenStreets]: [
		...
		new MiscMarker(`eOTNC`, EETypes.clue, [373.50680516628154, 388.36535534693917], `Klaus Part, transistor, shoot radio on the shelf with CRBR-S.`),
		new MiscMarker(`DwMIE`, EETypes.clue, [333.76018343302235, 467.30140318168293], `Klaus Part, antenna, shoot pylon with CRBR-S.`),
